class SchedulesController < ApplicationController
    ...
    def details
      ...
      cond = ScheduleEntry.scoped(:include => :project)
      if @project.nil?
        cond = cond.scoped :conditions => Project.allowed_to_condition(User.current, :view_schedules)
      end

      retrieve_date_range
      cond = cond.scoped(:conditions => ['date BETWEEN ? AND ?', @from, @to])

      ScheduleEntry.visible_by(User.current) do
        respond_to do |format|
          format.html {
            # Paginate results
            @entry_count = cond.count
            @entry_pages = Paginator.new self, @entry_count, per_page_option, params['page']
            @entries = cond.find(:all,
                                 :include => :user,
                                 :order => sort_clause,
                                 :limit  =>  @entry_pages.items_per_page,
                                 :offset =>  @entry_pages.current.offset)
            @total_hours = cond.sum(:hours).to_f
          ...
          format.atom {
            entries = cond.find(:all,
                                :include => :user,
                                :order => "#{ScheduleEntry.table_name}.date DESC",
